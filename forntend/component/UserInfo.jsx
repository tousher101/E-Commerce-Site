  ...
  const goAdminDashboard=()=>{
    ...
    return(
      <div className={`fixed top-0 left-0 h-full w-full bg-[rgba(0,0,0,0.5)] flex justify-center items-center z-50 transition-opacity duration-1000 ${design?'opacity-100':'opacity-0'}`}>
          <div className={`grid justify-items-center center content-center items-center bg-[#FCFCF7] p-[15px] rounded-2xl w-[350px] text-center text-black duration-1000 ${design?'scale-100':'scale-0'}`}>
           ...
           <div className="flex items-center gap-2 text-white mt-[25px]">
              <button onClick={openUploadImage} className=' h-[40px] w-[120px]  bg-green-500 rounded-xl  cursor-pointer'>Change Photo</button>
             {role==='USER'&&<button onClick={goUserDashboard} className=' h-[40px] w-[90px]  bg-blue-500 rounded-xl  cursor-pointer'>Manage</button>}
             {role==='ADMIN'&&<button onClick={goAdminDashboard} className=' h-[40px] w-[90px]  bg-blue-500 rounded-xl  cursor-pointer'>Manage</button>}
