...
export default function adminProduct({productPhotos, productName, productDescription, productPrice, productStock, productSize, productVariant, productColor, productWeight, update,create,
   productOriginalPrice,openEditModal,openDeleteModal, status, submitMakeTopSelling, submitMakePopular, submitClearAll
}){
            ...
            <div className="h-[90px] w-[150px] rounded-xl">
             ...
             <div className="grid grid-cols-1 items-center p-2 text-[13px]">
               <h1 className="flex gap-2">Size : {productSize.map((pro,i)=>(<span key={i}>{pro}</span>))}</h1>
                ...
                <h1 className="flex gap-2 break-words"> Variant : {productVariant.map((ver,i)=>(<span key={i}>{ver}</span>))} </h1>
