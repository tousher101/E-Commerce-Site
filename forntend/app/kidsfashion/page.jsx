...
export default  function kidsFashion(){
   ...
   const getProduct=async(page)=>{
    const res=await fetchWithAuth(`${BaseURI}/api/user/kidsfashion?page=${page}&limit=${16}`)
    setProductData(res.getKidsProduct);
    setTotalPage(res.totalPage);
    setAllproduct(res.totalKidsProduct)
   };
   useEffect(()=>{
    ...
    return(
        ...
        <div className=' mx-[10px] overflow-hidden'>
            <h1 className="text-center font-semibold text-3xl mt-[50px]">Kid's Product ({allproduct})</h1>
            <div className="grid grid-cols-4 gap-1">
            {productData?.map((pro)=>(
            <div key={pro.id}>
            <Product name={pro.name} description={pro.description} price={pro.price} stock={pro.stock} photos={pro?.photos[0]?.url}
            sold={pro.soldCount} comment={pro._count.comment} productId={pro.id} />
            </div>
            ))}
             </div>
