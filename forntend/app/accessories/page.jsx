...
export default function accessories() {
   ...
   const getProduct=async(page)=>{
    const res=await fetchWithAuth(`${BaseURI}/api/user/accessories?page=${page}&limit=${16}`)
    setProductData(res.getAccessoriesProduct);
    setTotalPage(res.totalPage);
    setAllproduct(res.totalAccessoriesProduct)
   };
   useEffect(()=>{
    ...
    return(
        ...
        <div className=' mx-[10px] max-w-[1380px] overflow-hidden'>
            <h1 className="ml-[20px] text-gray-500 font-bold text-3xl mt-[25px]">Accessories <span className="text-green-500"> Product ({allproduct})</span></h1>
            <div className="flex flex-wrap justify-center gap-1">
            {productData?.map((pro)=>(
            <div key={pro.id}>
            <Product name={pro.name} description={pro.description} price={pro.price} stock={pro.stock} photos={pro?.photos[0]?.url}
            sold={pro.soldeCount} comment={pro._count.comment} productId={pro.id} originalPrice={pro?.originalPrice} />
            </div>
            ))}
            </div>
