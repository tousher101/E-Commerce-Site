...
export default function shippedOrder(){
   ...
   const getOrderData=async()=>{
    const res=await fetchWithAuth(`${BaseURI}/api/admin/shippedorder`)
    setOrderData(res.getShippedorder);
            ...
            <h1 className="text-center text-gray-500 my-[20px] text-3xl font-bold"> Shipped <span className='text-green-500'>Order ({totalOrder})</span></h1>
                <div className='grid grid-cols-1 gap-1.5 items-center'>
                    {orderData?.length>0? orderData?.map((order)=>(
                        ...
                        userEmail={order?.user?.email} userPhone={order?.user?.phone} goDetails={()=>{goDetails(order?.id)}}/>
                        </div>
                         )) : <h1 className= 'text-center mt-50 text-4xl text-gray-500 font-semibold'> Shipped Order Not Available</h1>}
                           </div>
                       </div>
    )
}