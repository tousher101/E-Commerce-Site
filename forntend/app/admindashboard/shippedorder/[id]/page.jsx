...
export default function shippedOrderDetails(){
    ...
    const getDetailsData=async(id)=>{
        ...
        setDetailsData(res.shippedOrder);
        
    };

    const makeDeliverdOrder=async(id)=>{
        const res=await fetchWithAuth(`${BaseURI}/api/admin/makedeliverdorder/${id}`,{
            method:'PUT'
        });
        setMsg(res.msg);
        setType('Success');
        getAllCount();
            setTimeout(()=>{
            router.push('/admindashboard/shippedorder');
        },2000) 
    }
              ...
              <div className=" mx-[10px] my-[15px]"  data-aos='fade-up'>
               ...
               <div className="grid grid-cols-1 gap-1 border-1 p-3 rounded-xl border-gray-400 ">
                ...
                <h1>Tracking Link : <Link href={`${detailsData?.courier?.courierLink}`} ><span>{detailsData?.courier?.courierLink}</span></Link> </h1>
                 <h1>Tracking Number : {detailsData?.courier?.trackingNumber} </h1>
               </div>
                </div>
            </div>
