...
export default function returnOrders(){
     const BaseURI=process.env.NEXT_PUBLIC_API_URI;
      ...
      const [mode,setMode]=useState('today')
       ...
       const goDetails=(id)=>{
       router.push(`returnorder/${id}`)
      };
   
       const fetchData=async(page)=>{
       const endPoint= mode==='yesterday'?`${BaseURI}/api/admin/yesterdayreturnorders?page=${page}&limit=${20}`
       :mode==='week'?`${BaseURI}/api/admin/weeklyreturnorders?page=${page}&limit=${20}`
       :mode==='currentmonth'?`${BaseURI}/api/admin/monthlyreturnorders?page=${page}&limit=${20}`
       : mode==='year'?`${BaseURI}/api/admin/yearlyreturnorders?page=${page}&limit=${20}`
       :`${BaseURI}/api/admin/todayretunorders?page=${page}&limit=${20}`
      ...
      useEffect(()=>{
       ...
       const handelNext=()=>{
       if(totalPage>page){setPage((p)=>p+1)}else{setMsg('No More Orders Available'); setType('Error'); return}
      };
    return(
      <>
        {msg&&<Alert message={msg} type={type} onClose={()=>{setMsg('')}}/>}
        <div className=" mx-auto w-full overflow-hidden">
        <h1 className="text-center text-gray-500 my-[20px] text-3xl font-bold"> Return <span className='text-green-500'>Order ({totalOrder})</span></h1>
