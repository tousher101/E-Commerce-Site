...
const variantArray = Array.isArray(selectProduct?.variant)
  ...
  : selectProduct?.variant?.split(/[\s,]+/).filter(Boolean) || [];
   ...
   getPerfumeProduct();
    ...
    const getPerfumeProduct=async(page)=>{
        const res=await fetchWithAuth(`${BaseURI}/api/admin/adminperfume?page=${page}&limit=${20}`)
        setProductData(res.getPerfumeFashion);
        setTotalPage(res.totalPage);
        setTotalProduct(res.totalPerfumeFashion)
       
    };

      const submitSearch=async(barCode)=>{
      const res= await fetchWithAuth(`${BaseURI}/api/admin/searchproductbybarcodeprefume?barCode=${barCode}`)
    ...
    return(
        ...
        <div className=" w-full mx-auto overflow-hidden">
           <div className='flex justify-end gap-2 mx-[10px] items-center'>
            ...
            <h1 className='text-center text-3xl text-gray-400 font-semibold '>Perfume ({totalProduct})</h1>
