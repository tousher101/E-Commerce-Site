...
export default function allOrders(){
     const BaseURI=process.env.NEXT_PUBLIC_API_URI;
      ...
      const [mode,setMode]=useState('today')
       ...
       const goDetails=(id)=>{
       router.push(`allorder/${id}`)
      };
   
       const fetchData=async(page)=>{
       const endPoint= mode==='yesterday'?`${BaseURI}/api/admin/yesterdayallorders?page=${page}&limit=${20}`
       :mode==='week'?`${BaseURI}/api/admin/weeklyallorders?page=${page}&limit=${20}`
       :mode==='currentmonth'?`${BaseURI}/api/admin/monthlyallorders?page=${page}&limit=${20}`
       : mode==='year'?`${BaseURI}/api/admin/yearlyallorders?page=${page}&limit=${20}`
       :`${BaseURI}/api/admin/dailyallorders?page=${page}&limit=${20}`
    ...
    return(
      <>
        ...
        <h1 className="text-center text-gray-500 my-[20px] text-3xl font-bold"> All <span className='text-green-500'>Order ({totalOrder})</span></h1>
