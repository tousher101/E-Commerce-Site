...
export default function orderRequest(){
   ...
   const getOrderData=async()=>{
    const res=await fetchWithAuth(`${BaseURI}/api/admin/requestorder`)
    setOrderData(res.reqOrder);
    setTotalOrder(res.totalOrder)
   };

   const goDetails=(id)=>{
    router.push(`orderrequest/${id}`)
   };
   ...
   useEffect(()=>{
    ...
    return(
        <div className=" mx-auto w-full overflow-hidden">
            <h1 className="text-center text-gray-500 my-[20px] text-3xl font-bold">Order <span className='text-green-500'>Request ({totalOrder})</span></h1>
