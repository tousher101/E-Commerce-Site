...
export default function pedningOrderDetails(){
    ...
    const getDetailsData=async(id)=>{
        const res= await fetchWithAuth(`${BaseURI}/api/admin/getorderrequestdetails/${id}`)
        setDetailsData(res.orderReq);
        setMode(res.mode);
        
    };

    const cancelOrder=async(id)=>{
        const res=await fetchWithAuth(`${BaseURI}/api/admin/cancelorder/${id}`,{
            method:'PUT'
        });
        setMsg(res.msg);
        setType('Success');
        getAllCount();
        setTimeout(()=>{
            router.push('/admindashboard/orderrequest');
        },2000)   
    };

    const accepteOrder=async(id)=>{
        const res=await fetchWithAuth(`${BaseURI}/api/admin/acceptorder/${id}`,{
            method:'PUT'
        });
        setMsg(res.msg);
        setType('Success');
    ...
    <div className="mx-auto overflow-hidden grid grid-cols-1 w-full">
            <div className="grid grid-cols-1 justify-items-center mt-[15px]  mx-[10px]" data-aos='fade-up'>
                 ...
                 <div className="grid grid-cols-1 gap-2">
                    {detailsData?.items?.map((item)=>(
                        <div key={item.id}>
                        <ProductSummary mode={mode} name={item?.product?.name} photo={item?.product?.photos[0]?.url} size={item?.size} price={item?.unitPrice} quantity={item?.quantity} variant={item?.variant} color={item?.color}/>
                        </div>
                    ))}
         ...
         {mode==='ReqOrder'&&<div className="flex mx-[10px] justify-center gap-6 my-[30px] mb-[50px]"  data-aos='fade-up'>
            <button  onClick={openCancelModal}  className="border-1 bg-red-500 rounded-sm cursor-pointer p-2 text-white">Cancel Order</button>
            <button onClick={()=>{accepteOrder(detailsData.id)}}  className="border-1 bg-green-500 rounded-sm cursor-pointer p-2 text-white">Accepte Order</button>
        </div>}
